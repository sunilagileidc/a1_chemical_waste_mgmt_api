...
return new class extends Migration
{
    ...
    {
        Schema::create('pharmacist_wholesaler', function (Blueprint $table) {
            $table->bigIncrements('id')->comment('primary key');
            $table->unsignedBigInteger('pharmacist_id')->comment('Foreign key referencing pharmacist_details table');
            $table->string('name', 250)->comment('wholesaler name');
            $table->string('acc_no', 500)->comment('wholesaler account number');
